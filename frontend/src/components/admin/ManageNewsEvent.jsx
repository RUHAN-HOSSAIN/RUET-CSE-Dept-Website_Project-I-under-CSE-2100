...
import { NEWS_EVENT_CATEGORIES, NEWS_EVENT_FILTER_TYPES } from '../../constants/newsEventsData'
...
const ManageNewsEvent = () => {
  ...
  const [detailTarget, setDetailTarget] = useState(null) // ← popup এর জন্য
  ...
  const fetchStats = async () => {
    try {
      const { data } = await apiClient.get('/news-events/stats')
      setStats(data)
    } catch (err) {
      console.error(err)
    }
  ...
  return (
    <div className="relative bg-slate-100 min-h-full w-full font-poppins">
      ...
      {/* Modals */}
      {showAdd      && <AddItem    heading="News & Events" endpoint="/news-events" categories={NEWS_EVENT_CATEGORIES} inputId="img-add-input" setShowModal={setShowAdd} onSuccess={refresh}/>}
      {updateTarget && <UpdateItem heading="News & Events" endpoint="/news-events" item={updateTarget} categories={NEWS_EVENT_CATEGORIES} inputId="img-update-input" setShowModal={setUpdateTarget} onSuccess={refresh} />}
      ...
      <div className="px-6 sm:px-10 md:px-15 lg:px-20 py-10">
        ...
        <div className="flex items-center gap-2 mb-2 text-gray-400">
          ...
          &gt;
          <span className="text-blue font-medium text-sm sm:text-base">News & Events Management</span>
        </div>

        {/* Header */}
        <div className="flex items-center justify-between my-10 bg-white px-5 py-8 shadow-sm rounded-xl">
          <div>
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-dark-blue-2 leading-tight mb-1">News & Events</h1>
            <p className="text-gray-400  text-xs sm:text-sm">Add, Update or Delete News & Events</p>
          </div>
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          {NEWS_EVENT_FILTER_TYPES.map(f => (
            <button 
              key={f} 
              onClick={() => handleCategoryFilter(f)}
              className={`flex items-center gap-3 sm:gap-4 px-5 py-3 rounded-xl border text-sm font-medium transition-all cursor-pointer capitalize
                ${activeFilter === f ? 'bg-white border-blue text-blue shadow-sm' : 'bg-white border-gray-200 text-gray-500 hover:border-gray-400'}`}
            >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400">
                      • Updated: {formatDateToLocal(item.updatedAt)}
                    </span>
                  </div>
                </div>
...
export default ManageNewsEvent