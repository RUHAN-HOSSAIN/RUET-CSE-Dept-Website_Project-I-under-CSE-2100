...
const ManageAchievement = () => {
  ...
  const fetchStats = async () => {
    try {
      const { data } = await apiClient.get('/achievements/stats')
      setStats(data)
    } catch (err) {
      console.error(err)
    }
  ...
  return (
    <div className="relative bg-slate-100 min-h-full w-full font-poppins">
      ...
      {showAdd && <AddItem heading="Achievement" endpoint="/achievements" categories={ACHIEVEMENT_CATEGORIES} inputId="img-achievement-add" setShowModal={setShowAdd} onSuccess={refresh} />}
      {updateTarget && <UpdateItem heading="Achievement" endpoint="/achievements" item={updateTarget} categories={ACHIEVEMENT_CATEGORIES} inputId="img-achievement-update" setShowModal={setUpdateTarget} onSuccess={refresh} />}
